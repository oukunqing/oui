            ...
            setElemEvent: function (ddl) {
                ...
                $.addListener(elem, 'keydown', function (ev) {
                    ...
                    if (kc.inArray([KC.Enter, KC.Space, KC.Min.Enter])) {
                        ...
                        $.console.log('idx:', idx);
                        that.select(idx, {keyCode: kc, shortcut: false, panel: that.con}, ev.shiftKey || ev.ctrlKey);
                        return false;
    ...
    DropDownList.prototype = {
        ...
        select: function (num, arg, loop) {
            var that = this,
                ...
                //为什么这里要以0（而不是-1）作为循环切换，是因为索引本身已经+1
                idx = !$.isNumber(num) ? 0 : num <= 0 ? (loop ? len : 0) : num > len ? (loop ? 0 : len) : num;
