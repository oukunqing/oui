    ...
    $.extend($.fn, {
        formValidate: function (options) {
            ...
            var isElement = function (element) {
                ...
                setFormVerify = function (formElement, options, formData) {
                    var op = initFormConfig(formElement, options), configs = op.configs, list = [],
                        arr = ($.isObject(elements) && elements.length > 0 && !configs.dynamic) ? elements : op.formElement.getElementsByTagName(configs.tagName || "*"),
                        len = arr.length;
                    for (var i = 0; i < len; i++) {
            ...
            var element = findElement($(this)), len = $(this).length;
            ...
            //1. 赋值（不验证规则）
            if ($.isObject(formData)) {
                elements = setFormData(element, options, formData);
            }
            if ($.isObject(tableDatas)) {
                tableElements = setTableData(table, options, tableDatas);
            }
            //2.设置验证规则
