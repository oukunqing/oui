...
!function ($) {
    ...
    var doc = document,
        ...
        createTreeRow = function (that, container, datas, trees, pids, isAppend) {
            ...
            for (var i in datas) {
                ...
                if (isArray || pids.indexOf(treeData.pid) >= 0) {
                    ...
                    if (!isHead || !inserted) {
                        insertCheckboxCell(that, row, isHead, len, id);
                        insertLineNumberCell(that, row, isHead, len);
                        inserted = true;
                    }
        ...
        insertCheckboxCell = function (that, row, isHead, rowCount, rid) {
            var op = that.options;
            if (op.showCheckbox) {
                ...
                if (isHead) {
                    ...
                    cell.innerHTML = '<input type="checkbox" name="' + id + '" id="' + id + '-' + rid +'" />';
                }
            }
        },
    ...
    var setKeyValue = function (data, key, value) {
        ...
        buildSwitch = function (that, data, cell) {
            var op = that.options, 
                id = data.id,
                level = data.level,
            ...
            var cs = $.getElementSize(cell), ch = cs.style.height || cs.offset.height;
